public class test {
    static String ming="abcdefghijkopqrs";
    static String key="abcdefghijklmnop";
    static long w[]=new long[44];
    static long sp=1;
    static int rconi=0;
    static int m[][]={
            {0xc9,0xe5,0xfd,0x2b},
            {0x7a,0xf2,0x78,0x6e},
            {0x63,0x9c,0x26,0x67},
            {0xb0,0xa7,0x82,0xe5},
    };    static int mul[][]={{2,3,1,1},{1,2,3,1},{1,1,2,3},{3,1,1,2}};

    static int rcon[]={
            0x01000000,
            0x02000000,
            0x04000000,
            0x08000000,
            0x10000000,
            0x20000000,
            0x40000000,
            0x80000000,
            0x1b000000,
            0x36000000
    };

    static int pres[]={
            ,
    };
    static int s[][]=new int[16][16];
    public static void main(String[] args) {
        init();
        for(int i=4;i<8;++i){
            showw(w[i]);
        }



    }
    static void init(){
        for(int i=0;i<32;++i)
            sp=sp*2;
        sp--;//sp=0xffffffff

        int k=0;
        for(int i=0;i<16;++i){
            for(int j=0;j<16;++j){
                s[i][j]=pres[k++];
            }
        }

        //处理出w0-w3
        byte b[]=(key.getBytes());
        //w[0]=toint(b[0]);
        w[0]=0x2b7e1516;
        w[1]=0x28aed2a6;
        w[2]=0xabf71588;
        w[3]=0x09cf4f3c;
//        for(int i=1;i<=3;++i)
//            w[0]=(w[0]<<8)+toint(b[i]);
//        w[1]=toint((b[4]));
//        for(int i=5;i<=7;++i)
//            w[1]=(w[1]<<8)+toint(b[i]);
//        w[2]=toint((b[8]));
//        for(int i=9;i<=11;++i)
//            w[2]=(w[2]<<8)+toint(b[i]);
//        w[3]=toint((b[12]));
//        for(int i=13;i<=15;++i)
//            w[3]=(w[3]<<8)+toint(b[i]);
        for(int i=4;i<44;++i)
        {
            if(i%4!=0)
                w[i]=(w[i-1]^w[i-4])&sp;
            else{
                w[i]=(w[i-4]^(T(w[i-1])))&sp;
                rconi=(rconi+1)%16;
                //System.out.println(Long.toHexString(w[i]));
            }
        }
    }
    static void  showw(long a){
        System.out.print(Long.toHexString((a>>24))+" ");
        System.out.print(Long.toHexString((a>>16)&(0xff))+" ");
        System.out.print(Long.toHexString( (a>>8)&0xff)+" ");
        System.out.print(Long.toHexString((a&0xff))+" ");
        System.out.println();
    }
    static long T(long a){

       // System.out.println("prea:"+Long.toHexString(a));
        //System.out.println("a<<8:"+Long.toHexString((a<<8)&sp));
        //System.out.println("a&0xff:"+Long.toHexString(a>>24));

        System.out.println("prea:"+Long.toHexString(a));
        a=(a<<8)&sp^(a>>24);
        System.out.println("移位后:"+Long.toHexString(a));

        //System.out.println("a:"+Long.toHexString(a));
        long rt=0;
        long temp;
        temp=(a>>24);
        rt=((rt<<8)^subbyte(temp));
        temp=((a>>16)&0xff);
        rt=((rt<<8)^subbyte(temp));
        temp=((a>>8)&0xff);
        rt=((rt<<8)^subbyte(temp));
        temp=(a&0xff);
        rt=((rt<<8)^subbyte(temp));
        System.out.println("s盒变换后："+Long.toHexString(rt));
        return (rt&sp)^rcon[rconi];//////////////////
    }
    static long subbyte(long a){
        int x= (int) ((a>>4)&0xf);//前四位的int
        int y= (int) (a&0x0f);//后四位的int
        return s[x][y];
    }

    static int[][] SubByte(int m[][]){
        for(int i=0;i<4;++i)
        {
            for(int j=0;j<4;++j)
            {
                m[i][j]= s[q4(m[i][j])][h4(m[i][j])];
            }
        }
        return m;
    }

    static int q4(int a){
        return a>>4;
    }

    static int h4(int a){
        return a&(0x0f);
    }
    static void showm(){
        for(int i=0;i<4;++i){
            for(int j=0;j<4;++j){
                System.out.print(Integer.toHexString(m[i][j]));
                System.out.print(" ");
            }
            System.out.println();
        }
    }
    static int toint(byte a){
        int ans=0;
        int temp=1;
        for(int i=0;i<8;++i){
            ans+=temp*((a>>i)&1);
            temp=temp*2;
        }
        return ans;
    }
    static int [][] ShiftRows(int m[][]){
        int temp[][]=new int[4][4];
        for(int i=0;i<4;++i)
            for(int j=0;j<4;++j)
            {
                temp[i][j]=m[i][j];
            }
        m[1][0]=temp[1][1];m[1][1]=temp[1][2];m[1][2]=temp[1][3];m[1][3]=temp[1][0];
        m[2][0]=temp[2][2];m[2][1]=temp[2][3];m[2][2]=temp[2][0];m[2][3]=temp[2][1];
        m[3][0]=temp[3][3];m[3][1]=temp[3][0];m[3][2]=temp[3][1];m[3][3]=temp[3][2];
        return m;
    }

    static int[][] MixColumns(int m[][]){
        int rt[][]=new int[4][4];
        for(int i=0;i<4;++i) {
            for(int j=0;j<4;j++){
                // 3 0 1 2初值为1
                int tp=m[(i+3)%4][j];
                System.out.println(Integer.toHexString(tp));
                for(int k=0;k<4;++k) {
                    if(k!=((i+3)%4))
                        tp=(tp^(mix(mul[i][k],m[k][j])));
                }
                rt[i][j]=(tp&255);
            }
        }
        return rt;
    }

    static int m1(int b){
        return b;
    }

    static int m2(int b){
        if(((b>>7)&1)==0)
            return b<<1;
        else{
            return ((b<<1)&0xff)^27;
        }
    }
    static int[][] AddRoundKey(int m[][]){

        return m;
    }
    static int mix(int a,int b){
        if(a==1){
            return b;
        }
        else if(a==2){
            return m2(b);
        }
        else if(a==3){
            return m2(b)^m1(b);
        }
        return b;
    }
}
